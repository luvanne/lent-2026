...
//   1) 당일/전날/전전날만 오픈 대상
//   2) 당일은 바로 열림
//   3) 전날/전전날은 "당일 완료 후" 열림
//   4) 하루 오픈 수는 최대 3개
// - 당일 완료 시 전날/전전날 잠금 카드 시각 강조 효과
// ==============================================================================
...
const App = () => {
  ...
  const todayIndex = calendarData.findIndex((d) => d.date === todayMonthDay);
  const openWindowIndexes = todayIndex >= 0 ? [todayIndex, todayIndex - 1, todayIndex - 2].filter((i) => i >= 0) : [];
  ...
  const handleDayClick = async (index) => {
    ...
    if (!revealedDays[index]) {
      if (!openWindowIndexes.includes(index)) {
        setAlertMessage("당일/전날/전전날 여권만 열 수 있어요.");
        setTimeout(() => setAlertMessage(""), 3000);
        return;
      }

      if (index !== todayIndex && !completedDays[todayIndex]) {
        setAlertMessage("오늘 여권을 완료하면 전날/전전날 여권이 열려요.");
        setTimeout(() => setAlertMessage(""), 3000);
        return;
      }
      ...
      if (nextReveal.count >= maxDailyReveals) {
        setAlertMessage("하루에 최대 3개까지만 열 수 있어요.");
        setTimeout(() => setAlertMessage(""), 3000);
        return;
      }
      ...
      nextReveal.count += 1;
      setRevealCounts(nextReveal);
      await saveRevealCounts(nextReveal);

      nRev[index] = true;
      setRevealedDays(nRev);
      await saveToCloud(nRev, nComp, false);
      return;
    }
    ...
    if (isNew && index === todayIndex) {
      triggerBackPassportHighlight(index);
      setAlertMessage("오늘 완료! 전날/전전날 여권이 열렸어요 ✨");
      setTimeout(() => setAlertMessage(""), 2200);
    }

    await saveToCloud(nRev, nComp, isNew);
  };
  ...
  const markCompleted = async (index) => {
    ...
    await saveToCloud(nRev, nComp, true);
  };
      ...
      <main className="max-w-7xl mx-auto px-1">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3 md:gap-6">
          {calendarData.map((item, index) => {
            ...
            const isInOpenWindow = openWindowIndexes.includes(index);
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${isHighlighted ? 'ring-4 ring-fuchsia-300 shadow-[0_0_24px_rgba(217,70,239,0.45)] animate-pulse' : ''}
                ...
                >
                  <div className="flex flex-col items-center justify-between h-full py-4 px-2 text-[#e3c4ff] text-center relative">
                    ...
                    <div className="mb-2">
                      ...
                      <p className="text-[6px] md:text-[8px] font-serif tracking-wider opacity-80 mt-0.5">
                        {isTodayCard ? "TODAY" : isInOpenWindow ? (completedDays[todayIndex] ? "OPEN" : "TODAY FIRST") : "LOCKED"}
                      </p>
                    </div>
                    {!isClickable && (
              ...
              <button
                onClick={async () => {
                  if (result.type === 'prayer') {
                    await markCompleted(pendingCompleteIndex);
                    setPendingCompleteIndex(null);
                  }
                  setResult(null);
                }}
